...
#ifndef QPOPT_SMALBE_H
#define	QPOPT_SMALBE_H
...
namespace QPOpt {
	...
	Vector<Expression> smalbe(QPSettings *settings, Matrix<Expression> A, Vector<Expression> b, Vector<Expression> l, Matrix<Expression> B, Vector<Expression> x0){
		...
		binner = (-1)*binner;
		binner += b;
		x = mprgp_for_smalbe(settings, ArhoBTB, binner, l, x, B, rho, M);
	...
	template<typename Expression>
	Vector<Expression> smalbe(QPSettings *settings, Matrix<Expression> A, Vector<Expression> b, Vector<Expression> l, Matrix<Expression> B){
		Vector<Expression> x0 = b;
		x0(minlin::all) = 0.0; /* default initial approximation */  

		return smalbe(settings, A, b, l, B, x0);
	}
